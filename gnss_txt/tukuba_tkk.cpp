...
int open_TKK(void){
	...
	DeviceP = open(TKK,O_RDWR);
	if(DeviceP<0){
		// デバイスの open() に失敗したら
		printf("TKK Motion sensor open error !\n");
		exit(1);
	}
	set_serial();
  return 0;
...
{
  if(close(DeviceP)==-1){
    printf("TKK Motion sensor close error !\n");
    exit(1);  
  }
...
int get_navi_data(navidata* navi)
{
